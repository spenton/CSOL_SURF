;$Id: cos_viewer.pro,v 1.1 2019/03/19 19:40:15 spenton Exp spenton $
;
;+
;            cos_viewer
;
...
;   cos_viewer
;      or
;   cos_viewer,data,header
;      or
;   cos_viewer,file=filename
;
; OPTIONAL INPUTS:
;   data - data array
...
; =========================================================== SDI_OPENRAW
;
function sdi_openraw, info, coord=coscoord
   ...
   if n_elements(coscoord) eq 0 then begin
      ...
      info.wtitle+='  DETECT'
   endif else if coscoord eq -1 then begin
      ; use native
      widget_control, info.menuDispDetCoord,  sensitive=0
      widget_control, info.menuDispUserCoord, sensitive=1
      info.wtitle+='  DETECT'
   endif else if coscoord ne info.coscoord then begin
      ; requested a different coscoord than native file -> flip
      status = flipcoord(image=info.orig, x=(*info.data).xttag, y=(*info.data).yttag)
      info.coscoord=coscoord
   ...
   cos_viewer_display, info.orig, info=info
   ...
   if dtype eq 'HISTOGRAM' and detector eq 'NUV' then begin
...
; =========================================================== SDI_OPENFITS
;
function sdi_openfits, info, coord=coscoord

   ; if coscoord=-1 use native coord, if coscoord=0 use DETECT, if coscoord=1 use USER
   ...
   if strlen(msg) eq 0 then begin
      ...
      IF filetype ne "COS CUMULATIVE IMAGE" AND (instrument EQ 'COS' OR instrument EQ 'STIS' OR telescope EQ 'FUSE') THEN BEGIN
         xreadcos,nothing,hdr,orig,status,group=info.top_base,$
            filename=info.filename,ttag=ttag,xoff=xoff,yoff=yoff
         ; if an error occured in xreadcos return immediately
      ...
      ENDIF ELSE BEGIN
         ...
         if size(orig,/TNAME) eq 'STRUCT' then begin
            ...
            msg = 'Unrecognized data format for CEDAR'
            return,msg
         endelse
         info.xoff=0.0
         info.yoff=0.0
      ...
      ; check if data is FUV or NUV
      case strtrim(sxpar(header,'detector',/silent)) of
         '1A':  little_window_map = 1
         '1B':  little_window_map = 1
         '2A':  little_window_map = 1
         '2B':  little_window_map = 1
         'FUV': begin
            little_window_map = 1
            ; enable the menu for FUV detector
            widget_control, info.menuDispOtherSeg,  sensitive=1
            end
         'NUV': little_window_map = 0
         else: begin
            ;check for detector in instrument keyword
            detect = STRUPCASE(sxpar(header,'INSTRUME',/silent))
            if STRPOS(detect,'FUV') ne -1 then begin
              detector='FUV'
              little_window_map = 1
            endif else begin
              sz=size(*info.orig)
              if (sz[0] eq 2) and (sz[1] eq sz[2]) then $
                 little_window_map = 0 $
              else $
                 little_window_map = 1
            endelse
      ...
      endcase
      if little_window_map eq 0 then begin
         ; assume NUV data
         ...
         ; assume FUV data
         ; keep the same aspect ratio for the image with ysize=50
         info.little_ny = 47.0
         info.little_nx = ROUND((info.little_ny/maxy) * maxx)
         ; destroy previous instance of window before creating a new one
         if info.little_window ne -1 then widget_control, info.little_window, /DESTROY
      ...
      if instrument eq 'COS' then begin
         this_coscoord = sxpar((*info.header),'COSCOORD',count=count,/silent)
         if count gt 0 and strpos(this_coscoord[0],'USER') ge 0 then begin
            info.coscoord=1
            if n_elements(coscoord) eq 0 then begin
               ;no specified coord system -> use native
               widget_control, info.menuDispDetCoord,  sensitive=1
...
;
; cos_viewer Event Handler
; =========================================================== cos_viewer_EVENT
;
; cos_viewer Event Handler
;
pro cos_viewer_event,event

common cos_viewer_path, fits_path, raw_path
  ...
  case uvalue of
   ...
   'OPENCSOL': begin
         ...
         file = dialog_pickfile(title='Select CSOL/SURF  file(s)',/must_exist, path=csol_path, $
   ...
   'OPENFITS': begin
         ...
         if n_elements(fits_path) eq 0 then  begin
            fits_path = CEDAR_GETENV('CEDAR_FITS_ROOT')
         endif
   ...
   'OPENRAW': BEGIN
         ...
         if n_elements(raw_path) eq 0 then begin
            raw_path = CEDAR_GETENV('CEDAR_RAW_ROOT')
         endif
         ...
         ; When calling dialog_button, event for cos_viewer are still trying
   ...
   'PRINT':   BEGIN
         cos_viewer_ps, /PRINT, /REVERSED, info=info
         END

   'PS_BW':   BEGIN
         cos_viewer_ps, info=info
         END

   'PS_BWR':  BEGIN
         cos_viewer_ps, /REVERSED, info=info
         END

   'PS_COLOR': BEGIN
         cos_viewer_ps, /COLOR, info=info
         END

   'MK_PNG': BEGIN
         cos_viewer_ps, /COLOR, /PNG, /REVERSED, info=info
         END

   'MK_JPG': BEGIN
         cos_viewer_ps, /COLOR, /JPG, /REVERSED, info=info
         END
   ...
   'DISP_ORIENTATION' : BEGIN
      cos_viewer_plot_orient, info=info, /orient
      END

   'DISP_V2V3' : BEGIN
      cos_viewer_plot_orient, info=info, /v2v3
      END

   'DISP_PSA' : BEGIN
      cos_viewer_plot_orient, info=info, /psa
      END

   'DISP_X1DREF' : BEGIN
      cos_viewer_plot_x1d, info=info, /xtractab
      END

   'DISP_X1DDATA' : BEGIN
      cos_viewer_plot_x1d, info=info, /x1dfile
      END

   'DISP_X1DUSER' : BEGIN
      cos_viewer_plot_x1d, info=info, /x1duser
      END
   ...
   'DISP_ZOOM': begin
      ...
      cos_viewer_zoom, info.orig, zoom_img, info=info
      if PTR_VALID(info.zoom) then PTR_FREE,info.zoom
      info.zoom = PTR_NEW(zoom_img, /NO_COPY)
      cos_viewer_scale, info.zoom_id, info.zoom, imin=info.omin, imax=info.omax, scale=info.scale_type, ncolors=info.ncolors
   ...
   'DISP_REGION': cos_viewer_set_region,group=event.top,info=info
   ...
   'DISP_OTHERSEG' : BEGIN
      ...
      if strpos(strupcase(info.filename),".FITS") gt 0 then begin
         ...
         if strlen(msg) eq 0 then begin
            ...
            status = sdi_openfits(info, coord=info.coscoord)
      ...
      if size(*info.otherseg,/TNAME) eq 'STRUCT' then begin
         ...
         good = xttag_cos_phamask(*(*info.data).pha,*(*info.data).phamask)
         if good[0] ne -1 then begin
            ...
            if count eq n_elements(*(*info.data).xttag) then begin
               ...
               cos_viewer_display,select=pos, info=info
            endif
         endif else begin
            r=dialog_message('No events found',/error, dialog_parent=event.top)
            GOTO,SKIP
      ...
      if info.coscoord eq 1 then begin
         info.wtitle+='  USER'
      endif else if info.coscoord eq 0 then begin
         info.wtitle+='  DETECT'
      endif
      widget_control,info.top_base,tlb_set_title=info.wtitle
   ...
   'DATA_HEADER': begin
      if info.sdi_imgseq lt 0 then begin
         ...
         ShowText, info.filename, TITLE=info.filename, GROUP=event.top, INFOTEXT=text, $
                   HEIGHT=40, WIDTH=80,font='6X13', PRINTCMD=CEDAR_GETENV('CEDAR_PRINT_CMD')
      endif else begin
   ...
   'DATA_HIST' : cos_viewer_histogram,info.orig,TITLE=info.filename
   ...
   'BIG_WINDOW': begin
      ...
      if event.type eq 3 then begin
         cos_viewer_position, info=info
         GOTO,SKIP
      endif

      cos_viewer_xyval,info.orig,x,y,1,1,0,0,info=info
      if (info.cos_viewer_state[0] eq 0 and event.type eq 0 and event.press eq 4)  then begin
          ; the right mouse button was pressed to adjust the brightness/contrast
          info.cos_viewer_state[0] = 12
          widget_control,info.big_window,get_draw_view=v
          xr = ((x-v[0])>0)<(info.draw_window_size[0]-1)
          yr = ((y-v[1])>0)<(info.draw_window_size[1]-1)
          cos_viewer_stretchct, xr, yr, info=info, /getmouse
          GOTO,SKIP
      endif

      if (info.cos_viewer_state[0] eq 12 and event.type eq 1 and event.release eq 4)  then begin
          ; the right mouse button was released to stop adjustment of bright/contrast
          info.cos_viewer_state[0] = 0
          GOTO, SKIP
      endif

      if (info.cos_viewer_state[0] eq 12 and event.type eq 2)  then begin
      ...
      if (info.cos_viewer_state[0] eq 1) or (info.cos_viewer_state[0] eq 2) or $
         (info.cos_viewer_state[0] eq 9) or (info.cos_viewer_state[0] eq 10) or $
         (info.cos_viewer_state[0] eq 11) then  begin
            cos_viewer_defroi,event,info.big_id, info=info
      endif

      if ((info.cos_viewer_state[0] ge 3) and (info.cos_viewer_state[0] le 8)) or $
         (info.cos_viewer_state[0] eq 19) then begin
         cos_viewer_lineplot,event,'BIG', info=info
      endif

      if (event.press gt 1) or ((info.cos_viewer_state[0] eq 0) and $
         (event.press eq 1)) then begin
          ...
          if PTR_VALID(info.big_image) then $
             cos_viewer_zoom, info.big_image, zoom_img, info=info $
          else $
             cos_viewer_zoom, info.orig, zoom_img, info=info
          if PTR_VALID(info.zoom) then PTR_FREE,info.zoom
          info.zoom = PTR_NEW(zoom_img, /NO_COPY)
          cos_viewer_scale, info.zoom_id, info.zoom, imin=info.omin, imax=info.omax, $
   ...
   'LITTLE_WINDOW': begin
      ...
      cos_viewer_xyval,info.orig,x,y,factorx,factory,0,0,info=info

      if (info.cos_viewer_state[0] eq 1) or (info.cos_viewer_state[0] eq 2) or $
         (info.cos_viewer_state[0] eq 9) or (info.cos_viewer_state[0] eq 10) or $
         (info.cos_viewer_state[0] eq 11) then  $
            cos_viewer_defroi,event,info.little_id,info=info
      if ((info.cos_viewer_state[0] ge 3) and (info.cos_viewer_state[0] le 8)) or $
         (info.cos_viewer_state[0] eq 19) then $
         cos_viewer_lineplot,event,'LITTLE',info=info
      if (info.cos_viewer_state[0] eq 0) then begin
   ...
   'ZOOM_WINDOW': begin
      ...
      cos_viewer_xyval,info.orig,x,y,info.zoom_factor,info.zoom_factor,$
         info.xoffzoom, info.yoffzoom,info=info
      if (info.cos_viewer_state[0] eq 0) and (event.press ne 0) then begin
         if (event.modifiers eq 0) then begin
            widget_control,info.x_field,get_value=xx
            widget_control,info.y_field,get_value=yy
            cos_viewer_xyttag,xx,yy,/to_binned,xbin=(*info.data).xbin,ybin=(*info.data).ybin
      ...
      if (info.cos_viewer_state[0] eq 1) or (info.cos_viewer_state[0] eq 2) or $
         (info.cos_viewer_state[0] eq 9) or (info.cos_viewer_state[0] eq 10) or $
         (info.cos_viewer_state[0] eq 11) then  $
            cos_viewer_defroi,event,info.zoom_id,info=info
      if ((info.cos_viewer_state[0] ge 3) and (info.cos_viewer_state[0] le 8)) or $
         (info.cos_viewer_state[0] eq 19)  then $
            cos_viewer_lineplot,event,'ZOOM',info=info
      end
   ...
   'PLOT_CONTOUR': begin
      if (info.xoffzoom ge 0) and (info.yoffzoom ge 0) and $
         ...
         x = findgen(xend-info.xoffzoom+1)+info.xoffzoom
         y = findgen(yend-info.yoffzoom+1)+info.yoffzoom
         cos_viewer_xyttag,x,y,/frac, xbin=(*info.data).xbin, ybin=(*info.data).ybin, $
            xoff=info.xoff,yoff=info.yoff
   ...
   'PLOT_PHA': begin
      ...
      if val eq 'WHOLE IMAGE' then begin
         ...
         if PTR_VALID((*info.data).phamask) then begin
            ; combine the time and pha good data
            phagood = xttag_cos_phamask(*(*info.data).pha,*(*info.data).phamask)
         ...
         val=9   ;cos_viewer_state for box pha
         info.cos_viewer_state = [val,0,-1,-1]
         if info.cos_viewer_state[0] ne 2 then $
   ...
   'PLOT_WALK': begin
       ...
       mess = 'Place cursor on first corner and push left button'
       widget_control,info.sdimessage,set_value=mess
       info.cos_viewer_state=[walk,0,-1,-1]
   end
   ...
   'GAIN_MAP': begin
       ...
       if good[0] ne -1 then begin
          ...
          cos_viewer,gmap, wtitle=info.wtitle+'  Gain Map'
       endif else begin
          r=dialog_message('No events found within Valid Time range and PHA range',$
             /error,dialog_parent=event.top)
          GOTO,SKIP
   ...
   'GAUSS_EM': begin
      if (info.xoffzoom ge 0) and (info.yoffzoom ge 0) and $
         ...
         title=info.wtitle
         cos_viewer_gfit,data,x,y,1,title=title,group=event.top,xbin=(*info.data).xbin,ybin=(*info.data).ybin,$
   ...
   'GAUSS_AB': begin
      if (info.xoffzoom ge 0) and (info.yoffzoom ge 0) and $
         ...
         ;title=info.filename+'  '+info.sdiseg
         title=info.wtitle
         cos_viewer_gfit,data,x,y,0,title=title,group=event.top,xbin=(*info.data).xbin,ybin=(*info.data).ybin,$
            filename=info.filename, info=info
      endif
      end
   ...
   'TTAG': begin
      if (info.xoffzoom ge 0) and (info.yoffzoom ge 0) and $
         ...
         cos_viewer_xyttag,xr,yr, xbin=(*info.data).xbin, ybin=(*info.data).ybin, $
            xoff=info.xoff, yoff=info.yoff
         xttag_cos,info.filename,xr,yr,group=event.top, indata=info.data, topinfo=info
      endif
      end
       ...
       case val of
           ...
           'WHOLE IMAGE':  begin
              ...
              if NOT PTR_VALID(info.big_image) then begin
                 cos_viewer_stats,info.orig,minv,title='Image Statistics', group=event.top, info=info, $
                    filename=info.filename, exptime=info.exptime, ttag_time=ttag_time,region=region
              endif else  begin
                 cos_viewer_stats,info.big_image,minv,title='Image Statistics', group=event.top, info=info, $
                    filename=info.filename, exptime=info.exptime, ttag_time=ttag_time,region=region
              endelse
              widget_control,/hourglass
              end
           ...
           'ZOOMED IMAGE':  begin
              ...
              if (info.xoffzoom ge 0) and (info.yoffzoom ge 0) and $
                 ...
                 cos_viewer_stats,data,minv,title=title,group=event.top,filename=info.filename, $
...
;
   ...
   'PLOT' : BEGIN
       ...
       info.cos_viewer_State = [(val+2)<8,0,-1,-1,0]
       case info.cos_viewer_state[0] of
         ...
         8: begin
            ...
            info.cos_viewer_state[4] = widths[val-6]
   ...
   'FINDLINES': BEGIN
       ...
       info.cos_viewer_state = [(val+2),0,-1,-1,0]
       case info.cos_viewer_state[0] of
         7:  mess = 'Select first row and click left button'
...
;
pro cos_viewer_position, info=info
...
;============================================================ cos_viewer_PLOTS
pro cos_viewer_plots,xx,yy,color=color,overlay=overlay,$
...
;;============================================================ cos_viewer_PLOT_ORIENT
pro cos_viewer_plot_orient, info=info, orient=orient, v2v3=v2v3, psa=psa

; plot the orientation vector from the fits keyword if present
; in the little window only
;
   ; erase the plots first
   cos_viewer_scale,info.little_id,info.little_image,imin=info.omin,imax=info.omax,$
       scale=info.scale_type, ncolors=info.ncolors
   ...
   if info.disp_orient eq 1 then begin
      ...
      if count gt 0 then begin
         ...
         nx=x*cos(orientat/!radeg) - y*sin(orientat/!radeg)
         ny=x*sin(orientat/!radeg) + y*cos(orientat/!radeg)
         ; these are in usercoord -> change if viewing in detector
         if info.coscoord eq 0 then begin
   ...
   if info.disp_psa eq 1 then begin
      ...
      if info.coscoord eq 1 then status = flipcoord(x=nx,y=ny,/nuv)
...
;;============================================================ cos_viewer_PLOT_X1D
pro cos_viewer_plot_x1d, info=info, x1dfile=x1dfile, xtractab=xtractab, x1duser=x1duser
...
;
   ...
   if keyword_set(x1dfile) then begin
      if info.disp_x1dfile eq 1 then begin
         ; refresh the display
         if PTR_VALID(info.big_image) then $
            cos_viewer_scale, info.big_id, info.big_image, imin=info.omin, imax=info.omax, scale=info.scale_type, ncolors=info.ncolors $
         else $
            cos_viewer_scale, info.big_id, info.orig, imin=info.omin, imax=info.omax, scale=info.scale_type, ncolors=info.ncolors
         info.disp_x1dfile=0
      endif else begin
         info.disp_x1dfile=1
      endelse
   ...
   if info.disp_xtractab eq 1 or info.disp_x1duser eq 1 then begin
      ...
      if info.disp_x1duser eq 1 then begin
         ...
         this_coscoord = sxpar((*info.header),'COSCOORD',count=count,/silent)
         if count gt 0 and strpos(this_coscoord[0],'USER') ge 0 then begin
            refpath=CEDAR_GETENV("CEDAR_REFERENCE_USER_PATH")
         endif else begin
            refpath=CEDAR_GETENV("CEDAR_REFERENCE_PATH")
         endelse
         ...
         if strlen(x1dref[0]) eq 0 then begin
            ...
            if strpos(xtractab,'$lref') eq 0 then begin
               ...
               refpath = getenv('lref')
               if strlen(refpath) eq 0 then refpath = cedar_getenv('CEDAR_REFERENCE_USER_PATH')
   ...
   if displayit eq 1 then begin
      ...
      if count gt 0 and strpos(this_coscoord[0],'USER') ge 0 then begin
         ...
         if info.coscoord eq 1 then flipit=0 else flipit=1
      endif else begin
         ; original data was in detect coord
         if info.coscoord eq 1 then flipit=1 else flipit=0
      endelse
...
;=============================================================
;
pro cos_viewer_set_region_event,event
...
;
;========================================================== cos_viewer_SET_REGION
;
; widget to set display regions
;
pro cos_viewer_set_region,group=group, info=info
   ...
   widget_control,info.big_window,get_draw_view=v
   xlval = v[0] & ylval = v[1]
   cos_viewer_xyttag,xlval,ylval,xbin=(*info.data).xbin,ybin=(*info.data).ybin,$
      xoff=info.xoff, yoff=info.yoff
   ...
   xzval = info.xoffzoom
   yzval = info.yoffzoom
   cos_viewer_xyttag,xzval,yzval,xbin=(*info.data).xbin,ybin=(*info.data).ybin, $
      xoff=info.xoff, yoff=info.yoff
   ...
   xmanager,'cos_viewer_set_region',base,/no_block
   ...
   cos_viewer_xyttag,xzval,yzval,/to_binned, xbin=(*info.data).xbin, ybin=(*info.data).ybin
   ...
   info.zoom = PTR_NEW(zoom_img, /NO_COPY)
   cos_viewer_scale,info.zoom_id,info.zoom,imin=info.omin,imax=info.omax, scale=info.scale_type, ncolors=info.ncolors
   cos_viewer_xyttag,xlval,ylval,/to_binned, xbin=(*info.data).xbin, ybin=(*info.data).ybin
...
;
pro cos_viewer_display, image, select=select, info=info, reset=reset
...
;
   ...
   if keyword_set(reset) then begin
      ...
      ; this is used by xttag_cos when filtering and updating the data
      widget_control, info.top_base, set_uvalue=info
   endif

   cos_viewer_plot_orient, info=info
   cos_viewer_plot_x1d, info=info
...
;
pro cos_viewer_scale, window_id, image, xoff=xoff, yoff=yoff, noerase=noerase, $
...
;
pro cos_viewer_zoom,image,zoom_img,info=info
...
;
pro cos_viewer_xyval,image,x,y,zoomfactx,zoomfacty,xoff,yoff,info=info
   ...
   if (xx ge 0) and (xx le ns-1) and (yy ge 0) and (yy le nl-1) then begin
      if size(image,/TNAME) eq 'POINTER' then begin
         ...
         widget_control,info.val_field,set_value=float(image[xx,yy])
      endelse
      cos_viewer_xyttag,xx,yy, xbin=(*info.data).xbin, ybin=(*info.data).ybin, $
         xoff=info.xoff, yoff=info.yoff
...
;
pro cos_viewer_xyttag,x,y,to_binned=to_binned,frac=frac,xbin=xbin,ybin=ybin,xoff=xoff,yoff=yoff
;
...
;
; ======================================================== cos_viewer_HISTOGRAM
;
; Main Widget Driver for Histograms
;
pro cos_viewer_histogram,img,title=title
...
;
pro cos_viewer_gfit_event,event
   ...
   'SAVE': BEGIN
      ...
      path=CEDAR_GETENV('CEDAR_RESULTS_PATH')
...
;
pro cos_viewer_gfit,image,inx,iny,type,group=group,restore=restore,$
...
;
   ...
   cos_viewer_xyttag,x,y,/frac, xbin=xbin, ybin=ybin
...
;
   ...
   xmanager,'cos_viewer_gfit',base,/no_block
   return
end
;
; ============================================================= cos_viewer_STATS
...
pro cos_viewer_stats,indata,background,group=group,title=title,filename=filename,$
...
;
pro cos_viewer_defroi,event,window_id, info=info

;
; cos_viewer_State Vector
;   cos_viewer_State(0) = 0 not processing stats
;         1 Processing Box
;         2 Processing Defroi
;         9 Box PHA
;        10 Box X_WALK
...
; BOX Processing
;
   if info.cos_viewer_state[0] ne 2 then begin
;
; erase previous overlay
;
       if (info.cos_viewer_state[1] eq 0) or $
          ((info.cos_viewer_state[1] eq 1) and (info.xregion[1] eq -1)) then begin
          cos_viewer_plots,[info.cos_viewer_state[2],info.cos_viewer_state[2]],$
             [0,info.nl],color=255,/o,info=info
          cos_viewer_plots,[0,info.ns],[info.cos_viewer_state[3],info.cos_viewer_state[3]],$
             color=255,/o,info=info
          info.cos_viewer_state[2] = -1
          info.cos_viewer_state[3] = -1
       endif else begin
          ...
          cos_viewer_plots,xx,yy,color=255,/o,info=info
       endelse
       if event.press eq 0 then begin
          if info.cos_viewer_state[1] eq 0 then begin
             info.cos_viewer_state[2] = xdata
             info.cos_viewer_state[3] = ydata
             cos_viewer_plots,[info.cos_viewer_state[2],info.cos_viewer_state[2]],$
                [0,info.nl],color=255,/o,info=info
             cos_viewer_plots,[0,info.ns],[info.cos_viewer_state[3],info.cos_viewer_state[3]],$
                color=255,/o,info=info
       ...
       case info.cos_viewer_state[1] of
       0: begin         ;processing not started
         info.cos_viewer_state[1] = 1
...
;
       case info.cos_viewer_state[1] of
         ...
         1: begin
            ...
            if event.release gt 0 then info.cos_viewer_state[1] = 2
         end
       endcase
   endelse
;
...
;
   i1 = info.nregion-2
   i2 = info.nregion-1
   if (info.cos_viewer_state[0] eq 1)  or (info.cos_viewer_state[0] eq 9)  or $
      (info.cos_viewer_state[0] eq 10) or (info.cos_viewer_state[0] eq 11) then i1 = 0   ;draw entire box
   xx = info.xregion[i1:i2]
   yy = info.yregion[i1:i2]
   if (info.cos_viewer_state[0] eq 2) and (info.cos_viewer_state[1] eq 2) then begin  ;back to first point
...
; convert to window coordinates for all three windows and plot
;
   cos_viewer_plots,xx,yy,info=info
;
; process statistics
;
   if info.cos_viewer_state[1] lt 2 then return
   widget_control,info.sdimessage,set_value = ' '
   if info.cos_viewer_state[0] eq 10 or info.cos_viewer_state[0] eq 11 then begin
      ; requested a WALK PLOT
      widget_control,/hourglass
      if NOT PTR_VALID((*info.data).pha) then begin
         r = dialog_message('Pulse Height data not available', dialog_parent=event.top,/error)
      endif else begin
         cos_viewer_xyttag,info.xregion,info.yregion, xbin=(*info.data).xbin, ybin=(*info.data).ybin
         ...
         if PTR_VALID((*info.data).phamask) gt 0 then begin
            good = where((*(*info.data).xttag ge x1) and (*(*info.data).xttag le x2) and $
                 ...
                 xttag_cos_phamask(*(*info.data).pha, *(*info.data).phamask), ngood)
         endif else begin
            good = where((*(*info.data).xttag ge x1) and (*(*info.data).xttag le x2) and $
                 (*(*info.data).yttag ge y1) and (*(*info.data).yttag le y2) and $
         ...
         if ngood lt 1 then begin
            ...
            if info.cos_viewer_state[0] eq 10 then begin
               img = hist_2d((*(*info.data).xttag)[good],(*(*info.data).pha)[good],$
                  min1=x1,max1=x2,min2=0)
            endif else if info.cos_viewer_state[0] eq 11 then begin
            ...
            cos_viewer,img, wtitle=info.wtitle+'  Walk Plot'
            img=0b
         endelse
      endelse
   endif else if info.cos_viewer_state[0] eq 9 then begin   ;pha
      widget_control,/hourglass
      if NOT PTR_VALID((*info.data).pha) then begin
      ...
      if info.nregion gt 2 then begin
         ...
         widget_control,info.min_field,get_value=minv
         ttag_time=0.0
         if info.cos_viewer_state[0] eq 1 then begin
            ...
            ; create a temporary image to pass to cos_viewer_stats
         ...
         if n gt 1 then begin
            ...
            region = [x1,y1,x2,y2]
            cos_viewer_stats,areadata,minv,group=event.top, title=title, info=info, $
               filename=info.filename, exptime=info.exptime, ttag_time=ttag_time,region=region
            widget_control,/hourglass
         endif
      endif
...
;
pro cos_viewer_lineplot,event,window,info=info
;

; cos_viewer_State Vector
;
;   cos_viewer_State(0) = 3 Row
;                  4 Column
;         5 Row Sum
;         6 Column Sum
;         7 Linefind rowsum
;         8 Cross Section
;   cos_viewer_State(1) = Number of points measured
;
;
; If window not supplied, initialize cos_viewer_state vector, and print instructions
;
;
...
; erase previous overlay
;
   if info.cos_viewer_state[3] ge 0 then begin
       case info.cos_viewer_state(0) of
      3: cos_viewer_plots,[0,info.ns],[info.cos_viewer_state(3),info.cos_viewer_state(3)],$
         /o,color=255,info=info
      4: cos_viewer_plots,[info.cos_viewer_state(2),info.cos_viewer_state(2)],[0,info.nl],$
         /o,color=255,info=info
      5: cos_viewer_plots,[0,info.ns],[info.cos_viewer_state(3),info.cos_viewer_state(3)],$
         /o,color=255,info=info
      6: cos_viewer_plots,[info.cos_viewer_state(2),info.cos_viewer_state(2)],[0,info.nl],$
         /o,color=255,info=info
      7: cos_viewer_plots,[0,info.ns],[info.cos_viewer_state(3),info.cos_viewer_state(3)],$
         /o,color=255,info=info
      8: if info.xregion(1) ne -1 then begin
            xsection_corners,info.xregion(0),info.yregion(0), $
               info.xregion(1),info.yregion(1),info.cos_viewer_state(4),xcorn,ycorn,dist
            cos_viewer_plots,[xcorn,xcorn(0)],[ycorn,ycorn(0)],/o,color=255,info=info
         endif
      19: cos_viewer_plots,[info.cos_viewer_state(2),info.cos_viewer_state(2)],[0,info.nl],$
            /o,color=255,info=info
       endcase
   endif
...
; convert x and y to data coordinates
;
   cos_viewer_convert,event.x,event.y,window,x,y,info=info
   info.xregion[info.cos_viewer_state[1]] = x
   info.yregion[info.cos_viewer_state[1]] = y
   info.cos_viewer_state[2:3] = [x,y]
;
...
;
   if event.press ne 1 then begin
      ...
      7: cos_viewer_plots,[0,info.ns],[info.cos_viewer_state(3),info.cos_viewer_state(3)],$
         /o,color=255,info=info
      8: begin
         if info.cos_viewer_state[1] eq 1 then begin
            xsection_corners,info.xregion[0],info.yregion[0], $
                 info.xregion[1],info.yregion[1],info.cos_viewer_state[4],xcorn,ycorn,dist
            cos_viewer_plots,[xcorn,xcorn[0]],[ycorn,ycorn[0]],/o,color=255,info=info
         endif
...
; button pressed
;
   info.cos_viewer_state[1] = info.cos_viewer_state[1] + 1
;
; Row Plot
;
   if info.cos_viewer_state[0] eq 3 then begin
      cos_viewer_plots,[0,info.ns],[info.cos_viewer_state[3],info.cos_viewer_state[3]],line=1,info=info
      xrange = [0,info.ns]
      ...
      cos_viewer_xyttag,xrange,0,/frac, xbin=(*info.data).xbin, ybin=(*info.data).ybin, $
         xoff=info.xoff, yoff=info.yoff
      xv = findgen(info.ns)
      cos_viewer_xyttag,xv,0,/frac, xbin=(*info.data).xbin, ybin=(*info.data).ybin, $
         xoff=info.xoff, yoff=info.yoff
...
; Column Plot
;
   if info.cos_viewer_state[0] eq 4 then begin
      cos_viewer_plots,[info.cos_viewer_state(2),info.cos_viewer_state(2)],[0,info.nl],line=1,info=info
      xrange = [0,info.nl]
      if window eq 'ZOOM' then $
      ...
      cos_viewer_xyttag,0,xrange,/frac, xbin=(*info.data).xbin, ybin=(*info.data).ybin, $
         xoff=info.xoff, yoff=info.yoff
      xv = findgen(info.nl)
      cos_viewer_xyttag,0,xv,/frac, xbin=(*info.data).xbin, ybin=(*info.data).ybin, $
         xoff=info.xoff, yoff=info.yoff
...
; Row Sum
;
   if (info.cos_viewer_state(0) eq 5) or (info.cos_viewer_state(0) eq 7) then begin
      if (info.cos_viewer_state(1) eq 2) then begin
         cos_viewer_plots,[0,info.ns],[info.yregion(0),info.yregion(0)],/o,color=255,info=info
         cos_viewer_plots,[0,info.ns],[info.cos_viewer_state(3),info.cos_viewer_state(3)],line=1,info=info
         xrange = [0,info.ns]
         if window eq 'ZOOM' then $
            xrange = [info.xoffzoom,info.xoffzoom+info.zoom_width/info.zoom_factor]
         ...
         if info.cos_viewer_state(0) eq 5 then begin
...
; Column Sum
;
   if (info.cos_viewer_state(0) eq 6) or (info.cos_viewer_state(0) eq 19) then begin
      if (info.cos_viewer_state(1) eq 2) then begin
         cos_viewer_plots,[info.xregion(0),info.xregion(0)],[0,info.nl],/o,color=255,info=info
         cos_viewer_plots,[info.cos_viewer_state(2),info.cos_viewer_state(2)],[0,info.nl],$
            line=1,info=info
         xrange = [0,info.nl]
         if window eq 'ZOOM' then $
            xrange = [info.yoffzoom,info.yoffzoom+info.zoom_width/info.zoom_factor]
         ...
         if info.cos_viewer_state(0) eq 6 then begin
...
; x-section plot
;
   if (info.cos_viewer_state[0] eq 8) and (info.cos_viewer_state[1] eq 2) then begin
      xsection_corners,info.xregion[0],info.yregion[0], $
            info.xregion[1],info.yregion[1],info.cos_viewer_state[4],xcorn,ycorn,dist
      cos_viewer_plots,[xcorn,xcorn[0]],[ycorn,ycorn[0]],info=info
      if NOT PTR_VALID(info.big_image) then begin
         xsection_compute,info.orig,xcorn,ycorn,dist,info.cos_viewer_state[4],data
      endif else begin
         xsection_compute,info.big_image,xcorn,ycorn,dist,info.cos_viewer_state[4],data
      endelse
      cos_viewer_xyttag,info.xregion,info.yregion,/frac, xbin=(*info.data).xbin, ybin=(*info.data).ybin, $
         xoff=info.xoff, yoff=info.yoff
...
;
   if (info.cos_viewer_state[0] gt 4) and (info.cos_viewer_state[1] eq 1) then begin
      if (info.cos_viewer_state[0] eq 5) or (info.cos_viewer_state[0] eq 7) then $
         mess = 'Select last row and click left button' $
      else mess = 'Select last column and click left button'
      if info.cos_viewer_state[0] eq 8 then $
         mess = 'Select ending point and click left button'
      widget_control,info.sdimessage,set_value=mess
   endif else begin
      widget_control,info.sdimessage,set_value=info.scale_type + ' Display'
      info.cos_viewer_state = [0,0,-1,-1,0]
   endelse
...
;
pro cos_viewer_ps,color=color,reversed=reversed,print=print,png=png,jpg=jpg,info=info
...
;
pro cos_viewer_convert,xin,yin,window,xout,yout,to_screen=to_screen,info=info
;
...
;
   ...
   if (modifier eq 1) then begin
      ...
      y2 = y1 + 20L
      ; big window
      cos_viewer_scale, info.big_id, (*info.orig)[x1:x2,y1:y2], xoff=x1, yoff=y1, scale=info.scale_type, /noerase, ncolors=info.ncolors
      ; little window
      ...
      cos_viewer_scale,info.little_id,(*info.little_image)[x1:x2,y1:y2], xoff=x1, yoff=y1, scale=info.scale_type, /noerase, ncolors=info.ncolors
      ; zoom window
      cos_viewer_scale,info.zoom_id,info.zoom, scale=info.scale_type, ncolors=info.ncolors
    ...
    'table_menu': BEGIN   ; an option from the menubar was selected
       ...
       'File.Quit': BEGIN
            ...
            if PTR_VALID(info.topinfo.stars) then begin
               if (*info.topinfo.stars).n gt 0 then cos_viewer_display,info=info.topinfo
       ...
       'File.Print': BEGIN
            ...
            printf, unit, SYSTIME()
            print_cmd = CEDAR_GETENV('CEDAR_PRINT_CMD')
...
pro draw_stars, index=index, color=color, data=stars, topinfo=topinfo
   ...
   cos_viewer_plots, (*stars).x[index], (*stars).y[index],color=color,psym=1,symsize=1.0,info=topinfo
   ; draw circles
   A = FINDGEN(31) * (!PI*2.0/30.)
   usersym, cos(a), sin(a)
   cos_viewer_plots, (*stars).x[index],(*stars).y[index],color=color,psym=8,symsize=1.5,info=topinfo

end

;--------------------------------------------------------- stretch color table
pro cos_viewer_stretchct, brightness, contrast,  getmouse = getmouse, info=info
...
; inputs
   ...
   if (!d.n_colors GT 256) then begin
       if PTR_VALID(info.big_image) then begin
          cos_viewer_scale, info.big_id, info.big_image, imin=info.omin, imax=info.omax, $
              scale=info.scale_type, ncolors=info.ncolors, /noerase
       endif else begin
          cos_viewer_scale, info.big_id, info.orig, imin=info.omin, imax=info.omax, $
              scale=info.scale_type, ncolors=info.ncolors, /noerase
      endelse
      cos_viewer_scale,info.little_id,info.little_image,imin=info.omin,imax=info.omax,$
          scale=info.scale_type, ncolors=info.ncolors, /noerase
...
; ==================================================================
;
pro cos_viewer,image,h, file=file, FUV=fuv, NUV=nuv, PARENT=parent, RESTORE=restore, $
    CONTRAST=contrast, wtitle=wtitle

   ; define a non-existing CEDAR_DEF_FILE in case none were previously defined
   ; and we are running cos_viewer in "Virtual Machine" mode to prevent popup
   ; for request of cedar.def file all the time.
   defsysv, '!CEDAR_DEF_FILE', exists=exists
   if exists eq 0 then begin
      if lmgr(/runtime) then begin
         ; define the stuff that would normaly be in the cos_startup in case we're running VM
   ...
   endelse
    ...
    ; put some code here in case cos_viewer is used as a standalone program
   ...
   if n_elements(wtitle) eq 0 then wtitle='cos_viewer'
...
;
   ...
   val = strupcase(CEDAR_GETENV('CEDAR_FORCE_COORD'))
   ...
   endelse
   ;
   ; the variable data will replace the old cos_timetag_data COMMON BLOCK
   data = { $
      xttag:PTR_NEW(), $
      yttag:PTR_NEW(), $
      timetag:PTR_NEW(), $
   ...
   ;
   ; the variable info will replace the old cos_viewer_common COMMON BLOCK
   info = { $
      top_base: top_base,$
      base_fuv: base_fuv,$
      base_nuv: base_nuv,$
      ...
      big_id: big_id,$
      cos_viewer_state: cos_viewer_state,$
      xregion: xregion,$
      yregion: yregion,$
      nregion: nregion,$
   ...
   xmanager,'cos_viewer',top_base,/no_block
   ...
   if n_elements(file) ne 0 then begin
      ...
      if s ge 7 and s le 11 then begin
         ...
         info.little_id=little_id
         info.exptime=0.0
         cos_viewer_display,orig, info=info
         WIDGET_CONTROL, info.top_base, SET_UVALUE=info
      endelse
   endif
